...
\section{Metodologia}
    ...
    \subsection{Método de Eliminação de Gauss}
        ...
        \end{align}
            ...
            Como em \ref{eq:atualizacao} o termo $a^{(k)}_{k,k}$ (pivô) não pode ser nulo, pode ser necessário trocar a ordem de algumas das equações. Esta operação é o pivotamento parcial. Mesmo quando o termo não é nulo, é uma boa estratégia fazer o pivotamento para reduzir erros de arredondamento ($a^{(k)}_{k,k}$ muito pequeno). Diferentes estratégias podem ser aplicadas para escolher que equações que devem trocar de lugar:

            \begin{itemize}
                \item Quando $a^{(k)}_{k,k}=0$, trocar com a \emph{próxima} linha (menor $p$, tal que $k \leq p \leq n$) com $a^{(k)}_{p,k} \neq 0$ (Eliminação de Gauss com Substituição Regressiva).
                \item Trocar com a \emph{próxima} linha que maximiza o módulo do pivô: $|a^{(k)}_{p,k}|=\max_{k \leq i \leq n} |a^{(k)}_{i,k}|$ (Eliminação de Gauss com Pivotamento Parcial)
                \item Trocar com a \emph{próxima} linha que maximiza o módulo do pivô com relação aos elementos da sua linha: $\frac{|a^{(k)}_{p,k}|}{\max_{1 \leq j \leq n} |a_{p,j}|}=\max_{k \leq i \leq n} \frac{|a^{(k)}_{i,k}|}{\max_{1 \leq j \leq n} |a_{i,j}|}$ (Eliminação de Gauss com Pivotamento Parcial com Escala)
            \end{itemize}
            ...
            \begin{enumerate}
                ...
                \begin{enumerate}
                    \item Avaliar realizar pivotamento em função de algum dos critérios enumerados acima.
