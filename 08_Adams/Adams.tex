...
\section{Resultados}
        ...
        O mesmo teste foi feito na resolução do modelo de aquífero de Fetkovich. O número exato de avaliações com o método de Fetkovich foi 233, enquanto os demais métodos usaram aproximadamente 200 avaliações. A figura \ref{fig:testeAq} mostra que apenas os métodos de Adams-Moulton de 4 passos e Adams-Bashforth de 5 passos tiveram melhor desempenho que o método de Fetkovich nos tempos iniciais, que é quando acontecem as maiores vazões.

        As vazões de água nos tempos longos são baixas e de menor impacto no comportamento do reservatório. A a aparente degradação da resposta com o método de Fetkovich com o tempo perde importância. Foi feita a comparação em termos de produção acumulada de água (figura \ref{fig:testeAqWe}), e o método de Fetkovich se mostra com menores erros e mais estável que todos os demais.
